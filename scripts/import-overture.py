...
class OvertureImporter:
    ...
    def query_overture(self, states: Optional[list] = None,
        ...
        # Bounding-box filter enables parquet row-group pruning, which cuts
        # the S3 scan from the whole planet to just the covered partitions
        bbox_filter = ""
        if bbox:
    ...
    def reconnect_postgres(self):
        """Reconnect to Postgres to avoid connection timeout."""
        try:
            if self.pg_conn:
                self.pg_conn.close()
        except Exception:
    ...
    def upsert_batch(self, batch: list) -> tuple:
        ...
        insert_query = """
            INSERT INTO restaurants (
                ...
                primary_category, overture_update_date, source
            ) VALUES %s
    ...
    def run(self, states: Optional[list] = None, bbox: Optional[tuple] = None):
        ...
        try:
            ...
            query = self.query_overture(states, bbox)
            ...
            reconnect_interval = 10  # Reconnect every 10 batches to avoid timeout
            ...
            with tqdm(desc="Processing restaurants", unit=" rows") as pbar:
                while True:
                    ...
                    if len(batch) >= self.batch_size:
                        ...
                        # Proactively reconnect to avoid timeout
                        if batches_since_reconnect >= reconnect_interval:
                            logger.info(f"Reconnecting to database (processed {total_processed} rows)...")
                            self.reconnect_postgres()
                            batches_since_reconnect = 0
...
def main():
    ...
    parser.add_argument(
        '--bbox',
        help='Bounding box xmin,ymin,xmax,ymax (lon/lat) to prune the S3 scan (e.g., -87.7,24.4,-79.9,31.1 for FL)'
    )
    parser.add_argument(
        '--batch-size',
        type=int,
        default=2000,
        help='Batch size for database operations (default: 2000)'
    )
    parser.add_argument(
        '--database-url',
    ...
    if args.bbox:
        ...
        if len(parts) != 4:
            logger.error("--bbox requires exactly 4 comma-separated numbers: xmin,ymin,xmax,ymax")
            sys.exit(1)
        bbox = tuple(parts)

    try:
        count = importer.run(states=states, bbox=bbox)
